  ...
  "SubCategories" should {
    "aggregate SubCategories" in new WithApplication() {
      val request = FakeRequest(
        GET,
        "/subcategories/aggregate?sumRange=yyyy-mm&startDate=2014-01-01&endDate=2017-07-31&categories=house&subCategories=electricity,furniture%20and%20accessories,general,internet,phone,rent"
      )
      ...
      contentAsString(response) must /(
        "data"
      ) /# 0 /# 6 / ("furniture and accessories in")
      contentAsString(response) must /(
        "data"
      ) /# 0 /# 7 / ("furniture and accessories out")
    ...
    "compute total incoming flow per SubCategory" in new WithApplication() {
      val request = FakeRequest(
        GET,
        "/subcategories/in?sumRange=yyyy-mm&startDate=2014-01-01&endDate=2016-05-30&categories=work%20and%20training&subCategories=salary,travel,contribution,material,training,general"
      )
      val response = route(app, request).get
      status(response) must equalTo(OK)
      contentType(response) must beSome.which(_ == "application/json")
    ...
    "compute total outgoing flow per SubCategory" in new WithApplication() {
      val request = FakeRequest(
        GET,
        "/subcategories/out?sumRange=yyyy-mm&startDate=2014-01-01&endDate=2017-07-31&categories=work%20and%20training&subCategories=salary,travel,contribution,material,training,general"
      )
      val response = route(app, request).get
      status(response) must equalTo(OK)
      contentType(response) must beSome.which(_ == "application/json")
