  ...
  "Accounts" should {
    ...
    "read single account details hvb-depot sharing disabled" in new WithApplication() {
      val request = FakeRequest(
        GET,
        "/accounts/6?isSharingRatioEnabled=False&endDate=2015-12-31"
      )
      val response = route(app, request).get
      status(response) must equalTo(OK)
      contentType(response) must beSome.which(_ == "application/json")
    ...
    "read single account details dkb sharing disabled" in new WithApplication() {
      val request = FakeRequest(
        GET,
        "/accounts/7?isSharingRatioEnabled=False&endDate=2015-12-31"
      )
      val response = route(app, request).get
      status(response) must equalTo(OK)
      contentType(response) must beSome.which(_ == "application/json")
    ...
    "Timeseries" should {
      "retrieve timeseries for all accounts sharing enabled" in new WithApplication() {
        val request = FakeRequest(
          GET,
          "/accounts/timeseries?startDate=2014-01-01&endDate=2016-03-31&sumRange=YYYY-MM&accounts="
        )
        val response = route(app, request).get
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "application/json")
      ...
      "retrieve timeseries for all accounts sharing disabled" in new WithApplication() {
        val request = FakeRequest(
          GET,
          "/accounts/timeseries?isSharingRatioEnabled=False&startDate=2014-01-01&endDate=2016-03-31&sumRange=YYYY-MM&accounts="
        )
        val response = route(app, request).get
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "application/json")
      ...
      "retrieve timeseries for single account: db" in new WithApplication {
        val request = FakeRequest(
          GET,
          "/accounts/timeseries?startDate=2014-01-01&endDate=2016-03-31&sumRange=YYYY-MM&accounts=1"
        )
        val response = route(app, request).get
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "application/json")
      ...
      "retrieve timeseries for single account sharing enabled: dkb" in new WithApplication {
        val request = FakeRequest(
          GET,
          "/accounts/timeseries?startDate=2014-01-01&endDate=2016-03-31&sumRange=YYYY-MM&accounts=7"
        )
        val response = route(app, request).get
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "application/json")
      ...
      "retrieve timeseries for single account sharing disabled: dkb" in new WithApplication {
        val request = FakeRequest(
          GET,
          "/accounts/timeseries?isSharingRatioEnabled=False&startDate=2014-01-01&endDate=2016-03-31&sumRange=YYYY-MM&accounts=7"
        )
        val response = route(app, request).get
        status(response) must equalTo(OK)
        contentType(response) must beSome.which(_ == "application/json")
