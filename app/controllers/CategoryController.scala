...
@Singleton
class CategoryController @Inject() (implicit
    ec: ExecutionContext,
  ...
  def readCategories(): Action[AnyContent] = Action.async {
    async {
      val categories = await {
        ...
        Category(
          ...
          subCategories =
            List(Category(x._2.subCategory, x._2.color.getOrElse("#000000")))
        )
      }
      val nestedCategories = categories
        .groupBy { x => (x.name, x.color) }
        .map { x =>
          Category(
            x._1._1,
            x._1._2,
            x._2.map { y => y.subCategories }.flatten.toList
          )
        }
      val res = nestedCategories.map { y =>
        Json.toJson(y)(JsonFormats.categoryFmt)
      }
  ...
  /** Request example:
    * /categories/aggregate?sumRange=yyyy-mm&startDate=2014-01-01&endDate=2016-03-31&categories=house,other,finance,mobility,living,health,free
    * time&accounts=
    */
  def aggregateCategory(): Action[AnyContent] = Action.async { request =>
    async {
  ...
  def totalFlowCat(flow: String): Action[AnyContent] = Action.async { request =>
    async {
      val dateFormat = Formatter.normalizeDateFormat(
        request.getQueryString("sumRange").getOrElse("")
      )
