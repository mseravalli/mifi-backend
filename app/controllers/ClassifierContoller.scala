...
@Singleton
class ClassifierController @Inject() (implicit
    ec: ExecutionContext,
  ...
  /** Returns for the provided fields the categories associated otherwise
    * "other" "to categorize"
    * @return
    */
  def classifyTransactions(): Action[AnyContent] = Action.async { request =>
    async {
      val res: Seq[Int] = await { classify }

      Ok(
        Json.obj(
          "total" -> JsNumber(res.size),
          "successful" -> JsNumber(res.sum)
        )
      )
    }
  }
  ...
  def classify: Future[Seq[Int]] = async {
    ...
    val taggedClasses: Map[Tuple3[String, String, String], List[String]] =
      await {
        db.run(
          sql"""
        select trc.*, tac.tag
    ...
    val uncategorizedTransactions = await {
      db.run(
        Tables.Transactions
          .filter(t =>
            t.category === "other" && t.subCategory === "to categorize"
          )
          .result
      )
    }
    ...
    val classifiedTransactions: Seq[(Long, (String, String, Seq[String]))] =
      uncategorizedTransactions.map { t =>
        {
          // TODO: this maybe should be a map to solve issues with multiple equal matches.
          val classifications: Seq[(String, String, Seq[String])] =
            taggedClasses
              .map { c =>
                {
                  ...
                  val fields =
                    t.receiver.getOrElse("") + " " + t.purpose.getOrElse("")
          ...
          if (classifications.size > 1) {
            logger.warn(
              s"Found multiple matches for transacion ${t.id}: ${classifications.toString}"
            )
          }
    ...
    val updateTransactionQueries = classifiedTransactions.map { ct =>
      ...
      var query: List[
        slick.sql.SqlStreamingAction[Vector[Int], Int, slick.dbio.Effect]
      ] = List(
        sql"""
    ...
    val res = await {
      Future.sequence(updateTransactionQueries.map { q =>
